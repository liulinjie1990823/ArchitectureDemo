 ...
 * date 2018/10/24
 */
class VLayoutFragment : MainMvpBaseFragment<ViewBinding, VLayoutPresenter>(), IVLayoutView {
    ...
    private fun initAdAdapter(list: List<BabyHomeModuleItemVo?>): MyDelegateAdapter {
        ...
        return object : MyDelegateAdapter(mContext, layoutHelper, R.layout.item_baby_home_ad, 1, ViewType.AD) {
            ...
            override fun onBindViewHolder(holder: ViewHolderHelper, position: Int) {
                ...
                val height = ((DisplayHelper.SCREEN_WIDTH - dip2px(mContext, 40f)) * 120 / 670f + dip2px(mContext, 30f)).toInt()
                val width = -1
