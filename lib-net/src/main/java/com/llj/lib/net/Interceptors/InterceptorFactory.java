...
/**
 * ArchitectureDemo describe: author llj date 2018/6/12
 */
public class InterceptorFactory {
  ...
  //设置日志的Interceptor
  public static Interceptor HTTP_LOGGING_INTERCEPTOR = new HttpLoggingInterceptor()
    ...
    private HttpUrl checkUrl(String url) {
      ...
      if (null == parseUrl) {
        throw new RuntimeException(
            "You've configured an invalid url : " + (TextUtils.isEmpty(url) ? "EMPTY_OR_NULL_URL"
                : url));
      } else {
        return parseUrl;
      }
