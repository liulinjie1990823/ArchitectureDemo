 ...
 * <li>
 * BigDecimal.ROUND_CEILING,接近正无穷大的舍入模式。如果 BigDecimal 为正,则舍入行为与 ROUND_UP 相同;如果为负,则舍入行为与 ROUND_DOWN
 * 相同。注意,此舍入模式始终不会减少计算值。</li>
 * <li>
 * BigDecimal.ROUND_FLOOR,接近负无穷大的舍入模式。如果 BigDecimal 为正,则舍入行为与 ROUND_DOWN 相同;如果为负,则舍入行为与 ROUND_UP
 * 相同。注意,此舍入模式始终不会增加计算值。</li>
 * </p>
 *
 * @author liulj
 */
...
//    }
    ...
    /**
     * 设置四舍五入，并保留小数位数 00204.5455353保留两位是204.55
     *
    ...
    /**
     * 默认五舍六入(如果需要四舍五入,需要设置setRoundingMode(RoundingMode.HALF_UP); ) #.##(可以确保小数点左边不存在多余的0)1.126->1.13
     * 1.100->1.1 00.00(小数点后面至少有两位)1.126->01.13 000000.50823f使用默认转换后是0.51
     *
     * @param formatType
     * @param data
     *
     ...
     *
     * @param pattern 格式如00.00，##.00，区别是在格式位数多余的情况下##将会使多余的位数空着，00将会使多余的位数补上0。 如果number位数多，将会无条件舍去
     * @param number  要转换的数字
     *
     * @return 对应格式的字符串
     */
