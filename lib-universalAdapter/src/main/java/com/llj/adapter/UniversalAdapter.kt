...
abstract class UniversalAdapter<Item, Holder : XViewHolder> : ListObserver<Any?>,
    ...
    protected abstract fun onBindViewHolder(viewHolder: Holder, item: Item?, position: Int)

    protected fun onBindViewHolder(viewHolder: Holder, item: Item?, position: Int, payloads: List<*>) {
    }

    protected fun onBindHeaderViewHolder(holderX: XViewHolder, position: Int) {}
    protected fun onBindFooterViewHolder(holderX: XViewHolder, position: Int) {}
    fun onCreateDropDownViewHolder(parent: ViewGroup, itemType: Int): XViewHolder? {
        return onCreateViewHolder(parent, itemType)
    }

    fun onBindDropDownViewHolder(viewHolder: Holder, item: Item?, position: Int) {
    ...
    fun createDropDownViewHolder(parent: ViewGroup, viewType: Int): XViewHolder {
        val XViewHolder: XViewHolder
        XViewHolder = if (mItemLayouts.indexOfKey(viewType) >= 0) {
        ...
        XViewHolder.itemView.setTag(R.id.com_viewholderTagID, XViewHolder)
        return XViewHolder
    }

    fun bindViewHolder(XViewHolder: XViewHolder, position: Int) {
        if (headersCount == 0 && footersCount == 0) {
            //没有头部和底部
            val adjustedPosition = getAdjustedPosition(position)
            XViewHolder.itemView.setTag(R.id.com_viewholderIndexID, adjustedPosition)
            onBindViewHolder(XViewHolder as Holder, get(adjustedPosition), adjustedPosition)
        } else {
            if (isHeaderPosition(position)) {
                //前面的是header
                onBindHeaderViewHolder(XViewHolder, position)
            } else if (isFooterPosition(position)) {
                //后面的是footer
                onBindFooterViewHolder(XViewHolder, getAdjustedFooterPosition(position))
            } else {
                //item的位置
                val adjustedPosition = getAdjustedPosition(position)
                XViewHolder.itemView.setTag(R.id.com_viewholderIndexID, adjustedPosition)
    ...
    fun bindViewHolder(XViewHolder: XViewHolder, position: Int, payloads: List<*>) {
        if (headersCount == 0 && footersCount == 0) {
            //没有头部和底部
            val adjustedPosition = getAdjustedPosition(position)
            XViewHolder.itemView.setTag(R.id.com_viewholderIndexID, adjustedPosition)
            onBindViewHolder(XViewHolder as Holder, get(adjustedPosition), adjustedPosition, payloads)
        } else {
            if (isHeaderPosition(position)) {
                //前面的是header
                onBindHeaderViewHolder(XViewHolder, position)
    ...
    fun bindDropDownViewHolder(XViewHolder: XViewHolder, position: Int) {
        if (headersCount == 0 && footersCount == 0) {
            val adjustedPosition = getAdjustedPosition(position)
            XViewHolder.itemView.setTag(R.id.com_viewholderIndexID, adjustedPosition)
            onBindDropDownViewHolder(XViewHolder as Holder, get(adjustedPosition), adjustedPosition)
        } else {
            if (isHeaderPosition(position)) {
                onBindHeaderViewHolder(XViewHolder, position)
            } else if (isFooterPosition(position)) {
    ...
    fun getInternalItemViewType(position: Int): Int {
        val viewType: Int
        viewType = if (isHeaderPosition(position)) {
