...
public abstract class UniversalAdapter<Item, Holder extends ViewHolder>
  ...
  @Override
  public void setItemDoubleClickedListener(
      ItemDoubleClickedListener<Item, Holder> itemDoubleClickedListener) {
  ...
  protected abstract void onBindViewHolder(@NonNull Holder viewHolder, @Nullable Item item,
      int position);


  protected void onBindViewHolder(@NonNull Holder viewHolder, @Nullable Item item, int position,
      ...
      if (isHeaderPosition(position)) {
        ...
        if (mFooterClickListener != null) {
          mFooterClickListener
              .onFooterDoubleClicked(this, holder, getAdjustedFooterPosition(position));
        }
      } else {
        if (mItemDoubleClickedListener != null) {
          int adjusted = getAdjustedPosition(position);
          mItemDoubleClickedListener
              .onItemDoubleClicked(this, get(adjusted), (Holder) holder, adjusted);
        }
      }
    }
  }
    ...
    if (internalIsEnabled(position)) {
      if (isHeaderPosition(position)) {
        ...
        if (mFooterLongClickedListener != null) {
          return mFooterLongClickedListener
              .onFooterLongClicked(this, holder, getAdjustedFooterPosition(position));
        }
      } else {
        if (mItemLongClickedListener != null) {
          int adjusted = getAdjustedPosition(position);
          return mItemLongClickedListener
              .onItemLongClicked(this, get(adjusted), (Holder) holder, adjusted);
        }
      }
    }
    return false;
  ...
  protected final ListObserverListener<Item> observableListener = new ListObserverListener<Item>() {
    ...
    public void onItemRangeChanged(ListObserver<Item> observer, int start, int count,
      ...
      UniversalAdapter.this.onItemRangeChanged(start, count, payload);
    }

    @Override
    public void onItemRangeChanged(ListObserver<Item> observer, int start, int count) {
      UniversalAdapter.this.onItemRangeChanged(start, count);
    }

    @Override
    public void onItemRangeInserted(ListObserver<Item> observer, int start, int count) {
      UniversalAdapter.this.onItemRangeInserted(start, count);
    }

    @Override
    public void onItemRangeRemoved(ListObserver<Item> observer, int start, int count) {
      UniversalAdapter.this.onItemRangeRemoved(start, count);
    }
