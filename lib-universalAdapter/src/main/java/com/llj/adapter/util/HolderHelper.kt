...
class HolderHelper : IHolder {
    ...
    override fun <T : View> getView(@IdRes id: Int): T {
        val view = views[id] as T?
        if (view != null) {
            return view
        }
        val result = itemView.findViewById<View>(id) as T?
        if (result == null) {
    ...
    override fun setOnLongClickListener(@IdRes id: Int, onLongClickListener: View.OnLongClickListener?): IHolder {
