...
class StatusBarCompatKitKat {
    ...
    private static int getStatusBarHeight(@NonNull Window window) {
        int result = 0;
        int resId = window.getContext().getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resId > 0) {
     ...
     */
    private static View addFakeStatusBarView(@NonNull Window window, int statusBarColor, int statusBarHeight) {
     ...
     */
    private static void removeFakeStatusBarViewIfExist(@NonNull Window window) {
        ViewGroup mDecorView = (ViewGroup) window.getDecorView();
     ...
     */
    static void translucentStatusBar(@NonNull Window window) {
        window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        ...
        removeFakeStatusBarViewIfExist(window);
        removeMarginTopOfContentChild(mContentChild, getStatusBarHeight(window));
        if (mContentChild != null) {
            ViewCompat.setFitsSystemWindows(mContentChild, false);
        }
    }
    ...
    static void setStatusBarColorForCollapsingToolbar(@NonNull Window window,
        ...
        final View statusView = addFakeStatusBarView(window, statusColor, statusBarHeight);
