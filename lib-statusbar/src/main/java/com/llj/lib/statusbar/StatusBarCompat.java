...
public class StatusBarCompat {
     ...
     * @param alpha       0 - 255
     */
    public static void setStatusBarColor(@NonNull Window window, @ColorInt int statusColor, int alpha) {
        setStatusBarColor(window, calculateStatusBarColor(statusColor, alpha));
    }
    ...
    public static void setStatusBarColor(@NonNull Window window, @ColorInt int statusColor) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            StatusBarCompatLollipop.setStatusBarColor(window, statusColor);
    ...
    public static void setStatusBarColorForCollapsingToolbar(@NonNull Window window,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            StatusBarCompatLollipop.setStatusBarColorForCollapsingToolbar(window, appBarLayout, collapsingToolbarLayout, toolbar, statusColor);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            StatusBarCompatKitKat.setStatusBarColorForCollapsingToolbar(window, appBarLayout, collapsingToolbarLayout, toolbar, statusColor);
        }
    }
}
