   ...
   converting to extended RGB */
{
  ...
  if (maxval == MAXJSAMPLE) {
    if (aindex >= 0) GRAY_RGB_READ_LOOP(read_pbm_integer(cinfo, infile, maxval),
                                        ptr[aindex] = 0xFF;)
    else GRAY_RGB_READ_LOOP(read_pbm_integer(cinfo, infile, maxval),)
  } else {
    if (aindex >= 0) GRAY_RGB_READ_LOOP(rescale[read_pbm_integer(cinfo, infile, maxval)],
                                        ptr[aindex] = 0xFF;)
...
/* This version is for reading text-format PPM files with any maxval */
{
  ...
  ptr = source->pub.buffer[0];
  if (maxval == MAXJSAMPLE) {
    if (aindex >= 0) RGB_READ_LOOP(read_pbm_integer(cinfo, infile, maxval),
                                   ptr[aindex] = 0xFF;)
    else RGB_READ_LOOP(read_pbm_integer(cinfo, infile, maxval),)
  } else {
    if (aindex >= 0) RGB_READ_LOOP(rescale[read_pbm_integer(cinfo, infile, maxval)],
                                   ptr[aindex] = 0xFF;)
   ...
   and converting to extended RGB */
{
  ...
  bufferptr = source->iobuffer;
  if (maxval == MAXJSAMPLE) {
    if (aindex >= 0) GRAY_RGB_READ_LOOP(*bufferptr++, ptr[aindex] = 0xFF;)
    else GRAY_RGB_READ_LOOP(*bufferptr++,)
  } else {
    if (aindex >= 0) GRAY_RGB_READ_LOOP(rescale[UCH(*bufferptr++)], ptr[aindex] = 0xFF;)
...
/* This version is for reading raw-byte-format PPM files with any maxval */
{
  ...
  bufferptr = source->iobuffer;
  if (maxval == MAXJSAMPLE) {
    if (aindex >= 0) RGB_READ_LOOP(*bufferptr++, ptr[aindex] = 0xFF;)
    else RGB_READ_LOOP(*bufferptr++,)
  } else {
    if (aindex >= 0) RGB_READ_LOOP(rescale[UCH(*bufferptr++)], ptr[aindex] = 0xFF;)
