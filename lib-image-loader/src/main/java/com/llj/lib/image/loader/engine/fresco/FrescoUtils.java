...
public class FrescoUtils {
  ...
  public static void initFresco(final Context context, NetworkFetcher networkFetcher) {
    ...
    ImagePipelineConfig.Builder newBuilder = ImagePipelineConfig
  ...
  public static long getFrescoDiskCachesSize() {
    long size = Fresco.getImagePipelineFactory().getMainFileCache().getSize() + Fresco
        .getImagePipelineFactory().getSmallImageFileCache().getSize();
    if (size < 0) {
      size = 0;
    }
    return size;
  ...
  /**
   * 圆形,圆角切图 静态图可以使用setCornersRadius来设置圆角 gif的图需要使用setOverlayColor方法来设置圆角
   *
  ...
  /**
   * 图片显示的控制器,设置不同状态下的图片 mSimpleDraweeView.setHierarchy(hierarchy);
   *
   * @param context
   * @return
   */
  public static GenericDraweeHierarchy getGenericDraweeHierarchy(Context context) {
    GenericDraweeHierarchyBuilder genericDraweeHierarchyBuilder = new GenericDraweeHierarchyBuilder(
  ...
  public static ImageRequest getImageRequest(String url, int width, int height) {
    if (!TextUtils.isEmpty(url)) {
      ...
      ImageRequestBuilder imageRequestBuilder = ImageRequestBuilder
          .newBuilderWithSource(Uri.parse(filterUrl));
      ...
      imageRequestBuilder
          .setProgressiveRenderingEnabled(false);//是否逐行加载,主要用于渐进式的JPEG图,默认false,仅仅支持网络图
      imageRequestBuilder
          .setLocalThumbnailPreviewsEnabled(false);//是否用缩略图进行预览,本功能仅支持本地URI，并且是JPEG图片格式,默认false
   ...
   * @return
   */
  public static DraweeController getDraweeController(ImageRequest imageRequest,
  ...
  private static DraweeController getDraweeController(String url, int width, int height,
   ...
   */
  private static DraweeController getDraweeController(String url, int width, int height,
      boolean autoPlayAnimations, DraweeView view, BaseControllerListener<ImageInfo> listener) {
