...
/**
 * ArchitectureDemo.
 * 对所有跳转链接进行预处理，包括带有group的传统内页路径和外链（http,自定义scheme）
 *
 * @author llj
 * @date 2019-10-22
 */
...
public class LoginPretreatmentServiceImpl implements PretreatmentService {
    ...
    public static boolean start(Context context, String link, Postcard postcard) {
        ...
        //判断是否登录
        //通过ciw映射跳转到内页:ciw://InjectActivity?needLogin=1&type=1&name=2&Id=3
        ...
        if (link.startsWith(AppManager.getInstance().getJumpConfig().getNativeScheme())) {
            ...
            //通过WebView跳转:https://activity.jiehun.com.cn/app-update/index.html?hbh_app_version=6.10.0&hbh_app_link=ciw://mv/home?tab=0&needLogin=1
            if (params != null && params.containsKey(CJump.HBH_APP_VERSION) && params.containsKey(CJump.HBH_APP_LINK)) {
                String version = params.get(CJump.HBH_APP_VERSION) == null ? "0.0.0" : params.get(CJump.HBH_APP_VERSION);
                String appLink = params.get(CJump.HBH_APP_LINK) == null ? "" : params.get(CJump.HBH_APP_LINK);
                ...
                if (current >= target && decodeAppLink != null && decodeAppLink.startsWith(WebViewManager.getInstance().getWebViewConfig().getScheme())) {
                    //app版本比url中的版本大的
    ...
    private static void goToNativePage(String link, HashMap<String, String> params, Postcard postcard) {
    ...
    public static int calculation(String version) {
        ...
        if (split.length == 1) {
            ...
            num = AParseUtils.parseInt(split[0]) * 100 * 100 + AParseUtils.parseInt(split[1]) * 100 + AParseUtils.parseInt(split[2]);
        }
        return num;
