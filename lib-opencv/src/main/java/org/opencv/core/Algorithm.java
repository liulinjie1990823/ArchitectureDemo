 ...
 *
 * especially for classes of algorithms, for which there can be multiple implementations. The
 * examples are stereo correspondence (for which there are algorithms like block matching,
 * semi-global block matching, graph-cut etc.), background subtraction (which can be done using
 * mixture-of-gaussians models, codebook-based algorithm etc.), optical flow (block matching,
 * Lucas-Kanade, Horn-Schunck etc.).
 *
