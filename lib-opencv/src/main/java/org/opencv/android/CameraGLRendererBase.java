...
@TargetApi(15)
public abstract class CameraGLRendererBase implements GLSurfaceView.Renderer,
    ...
    protected abstract void setCameraPreviewSize(int width,
        int height); // updates mCameraWidth & mCameraHeight
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized (this) {
            ...
            if (texListener != null) {
                ...
                boolean modified = texListener
                    .onCameraTexture(texFBO[0], texDraw[0], mCameraWidth, mCameraHeight);
    ...
    private static int loadShader(String vss, String fss) {
        ...
        if (status[0] == 0) {
            Log.e("CameraGLRendererBase",
                "Could not compile vertex shader: " + GLES20.glGetShaderInfoLog(vshader));
        ...
        if (status[0] == 0) {
            Log.e("CameraGLRendererBase",
                "Could not compile fragment shader:" + GLES20.glGetShaderInfoLog(fshader));
            GLES20.glDeleteShader(vshader);
        ...
        if (status[0] == 0) {
            Log.e("CameraGLRendererBase",
                "Could not link shader program: " + GLES20.glGetProgramInfoLog(program));
            program = 0;
            return 0;
        }
        ...
        if (status[0] == 0) {
            Log.e("CameraGLRendererBase",
                "Shader program validation error: " + GLES20.glGetProgramInfoLog(program));
    ...
    private void initFBO(int width, int height) {
        ...
        GLES20
            .glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, width, height, 0, GLES20.GL_RGBA,
        ...
        GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0,
            GLES20.GL_TEXTURE_2D, texFBO[0], 0);
    ...
    protected synchronized void doStart() {
        ...
        if (mCameraWidth > 0 && mCameraHeight > 0) {
            setPreviewSize(mCameraWidth,
                mCameraHeight); // start preview and call listener.onCameraViewStarted()
        }
    }
    ...
    protected void setPreviewSize(int width, int height) {
        ...
        CameraTextureListener listener = mView.getCameraTextureListener();
        if (listener != null) {
            listener.onCameraViewStarted(mCameraWidth, mCameraHeight);
        }
    }
