            ...
            if (null == mEngineService) {
                ...
                try {
                    if (mEngineService.getEngineVersion() < MINIMUM_ENGINE_VERSION) {
                        ...
                        Log.d(TAG, "Calling using callback");
                        mUserAppCallback.onManagerConnected(
                            LoaderCallbackInterface.INCOMPATIBLE_MANAGER_VERSION);
                        return;
                    }
                    ...
                    if ((null == path) || (path.length() == 0)) {
                        if (!mLibraryInstallationProgress) {
                            ...
                            mUserAppCallback
                                .onPackageInstall(InstallCallbackInterface.NEW_INSTALLATION,
                                    InstallQuery);
                        } else {
                            InstallCallbackInterface WaitQuery = new InstallCallbackInterface() {
                                public String getPackageName() {
                            ...
                            mUserAppCallback
                                .onPackageInstall(InstallCallbackInterface.INSTALLATION_PROGRESS,
                                    WaitQuery);
                        }
                        return;
                    } else {
