...
public class JavaCamera2View extends CameraBridgeViewBase {
                ...
                new CameraCaptureSession.StateCallback() {
                    ...
                    public void onConfigured(CameraCaptureSession cameraCaptureSession) {
                        ...
                        try {
                            ...
                            mCaptureSession
                                .setRepeatingRequest(mPreviewRequestBuilder.build(), null,
                                    mBackgroundHandler);
    ...
    boolean calcPreviewSize(final int width, final int height) {
        ...
        try {
            ...
            Size frameSize = calculateCameraFrameSize(sizes_list, new JavaCameraSizeAccessor(),
                width, height);
            Log.i(LOGTAG,
                "Selected preview size to " + Integer.valueOf((int) frameSize.width) + "x" + Integer
                    .valueOf((int) frameSize.height));
            assert (!(frameSize.width == 0 || frameSize.height == 0));
            if (mPreviewSize.getWidth() == frameSize.width
                && mPreviewSize.getHeight() == frameSize.height) {
                return false;
            } else {
    ...
    protected boolean connectCamera(int width, int height) {
        ...
        try {
            ...
            if ((getLayoutParams().width == LayoutParams.MATCH_PARENT) && (getLayoutParams().height
