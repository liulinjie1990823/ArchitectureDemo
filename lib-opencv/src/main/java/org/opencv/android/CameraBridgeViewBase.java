...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public CameraBridgeViewBase(Context context, AttributeSet attrs) {
        ...
        TypedArray styledAttrs = getContext()
            .obtainStyledAttributes(attrs, R.styleable.CameraBridgeViewBase);
    ...
    public interface CvCameraViewListener {
         ...
         * a modified frame which needs to be displayed on the screen. TODO: pass the parameters
    ...
    /**
     * This method is provided for clients, so they can signal camera permission has been granted. The
     * actual onCameraViewStarted callback will be delivered only after setCameraPermissionGranted and
     * enableView have been called and surface is available
     */
     ...
     * onCameraViewStarted callback will be delivered only after setCameraPermissionGranted and
     * enableView have been called and surface is available
     */
     ...
     * delivery of frames even though the surface view itself is not destroyed and still stays on the
     * screen
     */
    ...
    /**
     * This method sets the maximum size that camera frame is allowed to be. When selecting size - the
     * biggest size which less or equal the size set will be selected. As an example - we set
                ...
                if (mScale != 0) {
                    canvas.drawBitmap(mCacheBitmap,
                        ...
                        new Rect((int) ((canvas.getWidth() - mScale * mCacheBitmap.getWidth()) / 2),
                            ...
                            (int) ((canvas.getHeight() - mScale * mCacheBitmap.getHeight()) / 2
                                + mScale * mCacheBitmap.getHeight())), null);
                } else {
                    canvas.drawBitmap(mCacheBitmap,
                        new Rect(0, 0, mCacheBitmap.getWidth(), mCacheBitmap.getHeight()),
    ...
    /**
     * This method is invoked shall perform concrete operation to initialize the camera. CONTRACT: as
     * a result of this method variables mFrameWidth and mFrameHeight MUST be initialized with the
     * size of the Camera frames that will be delivered to external processor.
     *
    ...
    /**
     * This helper method can be called by subclasses to select camera preview size. It goes over the
     * list of the supported preview sizes and selects the maximum one which fits both values set via
     * setMaxFrameSize() and surface frame allocated for this view
     *
    ...
    protected Size calculateCameraFrameSize(List<?> supportedSizes, ListItemAccessor accessor,
        ...
        int maxAllowedWidth =
            (mMaxWidth != MAX_UNSPECIFIED && mMaxWidth < surfaceWidth) ? mMaxWidth : surfaceWidth;
