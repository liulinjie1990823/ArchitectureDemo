...
public abstract class WlBaseMediaEncoder {
    ...
    private LogoRenderImpl wlGLRender;
    ...
    public void setRender(LogoRenderImpl wlGLRender) {
    ...
    static class WlEGLMediaThread extends Thread {
        ...
        private void onCreate() {
            if (isCreate && encoder.get().wlGLRender != null) {
                isCreate = false;
                encoder.get().wlGLRender.onSurfaceCreated((GL10) eglHelper.getEglContext().getGL(), null);
            }
        }
        ...
        private void onChange(int width, int height) {
            if (isChange && encoder.get().wlGLRender != null) {
                isChange = false;
                encoder.get().wlGLRender.onSurfaceChanged((GL10) eglHelper.getEglContext().getGL(), width, height);
            }
        }

        private void onDraw() {
            if (encoder.get().wlGLRender != null && eglHelper != null) {
                encoder.get().wlGLRender.onDrawFrame((GL10) eglHelper.getEglContext().getGL());
                if (!isStart) {
                    encoder.get().wlGLRender.onDrawFrame((GL10) eglHelper.getEglContext().getGL());
                }
