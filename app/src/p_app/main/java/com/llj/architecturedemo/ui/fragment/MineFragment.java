...
public class MineFragment extends
    MainMvpBaseFragment<ViewBinding, PersonalCenterPresenter> implements
    IMineView {
  ...
  @BindView(R2.id.scrollView) NestedScrollView mScrollView;

  @BindViews({R2.id.iv_scanning,
      R2.id.iv_qr_code,
      R2.id.iv_message,
      R2.id.iv_setting,}) List<ImageView> mIvWhite;

  @BindViews({R2.id.iv_scanning1,
      R2.id.iv_qr_code1,
      R2.id.iv_message1,
  ...
  @BindView(R2.id.iv_header)         SimpleDraweeView mIvHeader;
  @BindView(R2.id.rl_login_wrap)     RelativeLayout   mRlLoginWrap;
  @BindView(R2.id.tv_login)          TextView         mTvLogin;
  @BindView(R2.id.iv_member_tag)     TextView         mIvMemberTag;
  @BindView(R2.id.tv_diamond_points) TextView         mTvDiamondPoints;
  @BindView(R2.id.tv_sign_in)        Button           mTvSignIn;
  @BindView(R2.id.tv_sign_tag)       TextView         mTvSignTag;

  @BindView(R2.id.ll_header_menu)   LinearLayoutCompat mLiHeaderMenu;
  @BindView(R2.id.li_mine_top_menu) LinearLayout       mLiMineTopMenu;

  @BindView(R2.id.cv_image1) CardView         mCvImage1;
  @BindView(R2.id.iv_add1)   SimpleDraweeView mIvAdd1;

  @BindView(R2.id.ll_tools) LinearLayout mLiTools;

  @BindView(R2.id.refresh_layout) JHSmartRefreshLayout mRefreshLayout;
  @BindView(R2.id.parallax)       ImageView            mParallax;
  @BindView(R2.id.iv_top_bg)      ImageView            mIvTopBg;
  ...
  public void onHiddenChanged(boolean hidden) {
    ...
    if (!hidden) {
      ...
      LightStatusBarCompat
          .setLightStatusBar(((Activity) mContext).getWindow(), !(mScrollView.getScrollY() == 0));
    }
  }
      ...
      public void onScrollChange(NestedScrollView nestedScrollView, int scrollX, int scrollY,
        ...
        if (percent > 0) {
          if (percent >= 1) {
            ...
            //上层的title
            mFlToolbarRoot.getBackground().mutate()
                .setAlpha((int) (getMiddle(percent, 0, 1) * 255));
            for (ImageView imageView : mIvWhite) {
  ...
  private void handleTopMenu() {
    ...
    topMenuVos.add(new MenuVo(R.drawable.ic_my_cash_coupon, R.string.cash_coupon,
        CRouter.APP_MINE_MY_COUPON_ACTIVITY, AppAction.MINE_CASH));
    topMenuVos.add(
        new MenuVo(R.drawable.ic_order, R.string.order, CRouter.ORDER_LIST, AppAction.MINE_ORDER));
    topMenuVos.add(new MenuVo(R.drawable.ic_comment, R.string.comment, CRouter.MY_COMMENT_LIST,
        AppAction.MINE_EVALUATE));
    topMenuVos.add(
        new MenuVo(R.drawable.ic_my_wallet, R.string.my_wallet, CRouter.APP_MINE_MY_WALLET_ACTIVITY,
            AppAction.MINE_WALLET));
  ...
  private void handleSign(PersonalCenterVo homePageVo) {
    ...
    if (activityVo.is_start() && !isEmpty(activityVo.getCorner()) && AViewUtils
        .isVisible(mTvSignIn)) {
  ...
  //头部菜单adapter
  public class HeaderMenuAdapter extends
      ListBasedAdapter<PersonalCenterCountVo.HeaderMenu, ViewHolderHelper> {
    ...
    @Override
    protected void onBindViewHolder(ViewHolderHelper viewHolder,
        final PersonalCenterCountVo.HeaderMenu data, final int position) {
  ...
  public class TopMenuAdapter extends ListBasedAdapter<MenuVo, ViewHolderHelper> {
    ...
    @Override
    protected void onBindViewHolder(ViewHolderHelper viewHolder, final MenuVo item,
        final int position) {
  ...
  public class ToolsAdapter extends ListBasedAdapter<PersonalCenterVo.ToolsVo, ViewHolderHelper> {
    ...
    protected void onBindViewHolder(@NonNull ViewHolderHelper viewHolder,
        final PersonalCenterVo.ToolsVo item, final int position) {
      viewHolder.setText(R.id.tv_title, item.getTitle());
      RecyclerView recyclerView = viewHolder.getView(R2.id.rv_tools_menu);
  ...
  //小工具adapter
  public class ToolsItemAdapter extends
      ListBasedAdapter<PersonalCenterVo.ToolsMenuVo, ViewHolderHelper> {
    ...
    @Override
    protected void onBindViewHolder(ViewHolderHelper viewHolder,
        final PersonalCenterVo.ToolsMenuVo item, final int position) {
  ...
  public class AdAdapter extends ListBasedAdapter<PersonalCenterVo.AdVo, ViewHolderHelper> {
    ...
    @Override
    protected void onBindViewHolder(ViewHolderHelper viewHolder, final PersonalCenterVo.AdVo item,
        final int position) {
